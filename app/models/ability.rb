class Ability
  ...
  def initialize(user)
    ...
    if user.admin?
      ...
      can :update, [Address,Information], :real_estate=>{:state => 'editing'}
      can :update, [Address,Information], :real_estate=>{:state => 'in_review'}
      cannot :update, [Address,Information], :real_estate=>{:state => 'published'}
    end
