...
describe Figure do
  describe 'initialize with invalid attributes' do
    before :each do
      @real_estate = Fabricate.build(:real_estate, :figure => Figure.new(
          :rooms => '2-3.5',
          :floor => 'EG',
    ...
    it 'has 9 errors' do
      @figure.valid?
      @figure.errors.should have(9).items
    end

    context 'a real estate with commercial utilization' do
      before :each do
      ...
      it 'has 10 errors' do
        @figure.valid?
        @figure.errors.should have(10).items
      end
    end
  end  
end
