...
describe Reference do
  context 'with saved reference in database' do
    ...
    it "shouldn't be possible to save the reference with the same keys" do
      @real_estate = Fabricate.build(:published_real_estate, :address => Fabricate.build(:address, :reference => Fabricate.build(:reference, reference_attributes)), :category => Fabricate(:category))
      expect(@real_estate.save).to be_true
